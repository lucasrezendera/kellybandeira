      ...
      <div className="flex flex-wrap gap-3 pb-6 border-b border-stone-100 justify-center lg:justify-start">
        {categories.map((cat) => (
          <button
            ...
            className={`flex items-center gap-3 px-6 py-4 rounded-lg text-sm font-bold uppercase tracking-wide transition-all duration-300 flex-grow md:flex-grow-0 justify-center ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {currentCategory?.services.map((service) => (
          <button
            ...
            className={`text-left p-6 rounded-xl border-2 transition-all duration-300 group flex justify-between items-start h-full ${
          ...
          >
            ...
            <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors mt-1 ${
              selectedService?.id === service.id ? 'bg-brand-600 text-white' : 'bg-white text-stone-300 group-hover:text-brand-400 border border-stone-200'
            }`}>
